
# JENSEN N. HU, MPH

Email: Jensennhu@gmail.com  
Location: New York City, NY  
GitHub: https://github.com/jensennhu

---

## Education

**Boston University School of Public Health**  
Master of Public Health, Epidemiology (2014–2016)  

**Boston University**  
Bachelor of Science, Health Science (2010–2014)

---

## Work Experience

### Mathematica — Senior Data Analytics Developer 
June 2022 – March 2025

- Collaborated with multidisciplinary research teams to deliver rapid-turnaround analyses and reporting for government stakeholders across workforce, education, and social policy programs. Program involved: Healthy Marriage and Responsible Fatherhood (HMRF), Regional Educational Laboratory (REL) for the Mid-Atlantic region, Retaining Employment and Talent After Injury/Illness Network (RETAIN), Analysis of Strategies for Expanding Apprenticeship (ASEA), School Meals Operations (SMO), Adult National Directory of New Hires (NDNH)
- Developed and maintained data processing pipelines for federal program evaluations using R, Stata, SAS, and Python, integrating administrative and survey datasets and automating reporting workflows producing statistical tables and metrics
- Designed data cleaning scripts for multi-state administrative datasets

---

### NYC Department of Health & Mental Hygiene — City Research Scientist III  
Sept 2021 – May 2022

- Analyzed mental health burden in NYC public schools using Python and SPARCS ED visit data
- Reported and archived COVID-19 vaccination coverage using SQL Server and Python
- Responded to ad‑hoc analytics requests from medical unit stakeholders
- Enhanced legacy SAS programs identifying severe asthma risk in schools

---

### NYC Department of Health & Mental Hygiene — City Research Scientist II  
Jan 2019 – Sept 2021

- Developed reusable reporting scripts in R and SAS
- Managed data for three mental health initiatives across 500+ schools
- Delivered program metrics to leadership

---

### NYC Data Science Academy — Data Science Fellow  
July 2018 – Sept 2018

- Built an R Shiny dashboard visualizing CDC outbreak data
- Scraped 9,000+ Microsoft App Store reviews using Python Scrapy
- Built predictive models for Ames Housing Kaggle competition

---

### Harvard Pilgrim Health Care Institute — Research Analyst I  
Apr 2016 – June 2018

- Executed distributed research network queries across clinical datasets
- Built ICD‑9, ICD‑10, NDC, and RxNorm code lists
- Aggregated large SAS datasets and produced epidemiologic reports
- Developed reusable SAS cohort identification tool (PCORnet PMP1)

---

## Publications

Hu, J.N. et al. *Developing Efficient Reusable SAS Programs in PCORnet: PMP1.*  
AMIA Informatics Summit, San Francisco (2018)

---

## Skills

**Languages:** R, Stata, SAS, Python, SQL  
**Tools:** Jupyter, RStudio, SQL Server, Tableau, Git, Excel
